...
# -*- coding: utf-8 -*-
"""
AUDITORIA DE ENCODING - AGENTE TURISMO IA
==========================================
Version simplificada sin emojis para Windows
"""
...
def audit_python_files():
        ...
        try:
            ...
            if has_spanish and not has_encoding_declaration:
                issues.append({
                    ...
                    'issue': 'Tiene caracteres españoles sin declaracion de encoding'
                })
    ...
    if issues:
        print(f"[WARNING] Encontrados {len(issues)} archivos con problemas potenciales:")
        for issue in issues:
            print(f"   - {issue['file']}: {issue['issue']}")
    else:
        print("[OK] Todos los archivos Python tienen encoding correcto")
    
    return issues

def audit_pdfs():
    """Verifica encoding en PDFs del directorio uploads."""
    print("\n[PDF] 2. AUDITORIA DE PDFs PROCESADOS")
    print("-" * 70)
    
    try:
        from pypdf import PdfReader
    except ImportError:
        print("[ERROR] pypdf no instalado. Ejecutar: pip install pypdf")
        return []
    
    uploads_dir = Path(__file__).parent.parent.parent / "uploads"
    
    if not uploads_dir.exists():
        print("[INFO] No existe directorio 'uploads' todavia")
        return []
    
    pdf_files = list(uploads_dir.glob("*.pdf"))
    
    if not pdf_files:
        print("[INFO] No hay PDFs en el directorio uploads")
        return []
    
    print(f"[INFO] Analizando {len(pdf_files)} archivos PDF...")
    ...
    for pdf_path in pdf_files:
        try:
            ...
            if '�' in first_page_text:
                ...
                print(f"   [WARNING] {pdf_path.name}: Caracteres corruptos detectados")
            else:
                print(f"   [OK] {pdf_path.name}: OK ({total_pages} paginas)")
            ...
            print(f"   [ERROR] {pdf_path.name}: Error - {str(e)}")
    
    if not issues:
        print("\n[OK] Todos los PDFs se procesan correctamente")
    
    return issues

def audit_chromadb():
    """Verifica encoding en la base de datos vectorial."""
    print("\n[CHROMADB] 3. AUDITORIA DE CHROMADB")
    print("-" * 70)
    ...
    if not db_path.exists():
        print("[INFO] ChromaDB no inicializada todavia")
        return []
    ...
    try:
        ...
        if not collections:
            print("[INFO] No hay colecciones en ChromaDB")
            return []
        ...
        for collection in collections:
            print(f"\n   Analizando coleccion: {collection.name}")
            
            # Obtener todos los documentos
            results = collection.get(include=['documents', 'metadatas'])
            
            if not results or not results['documents']:
                print(f"   [INFO] Coleccion vacia")
                continue
            
            doc_count = len(results['documents'])
            print(f"   [INFO] Documentos: {doc_count}")
            ...
            if corrupted_docs > 0:
                print(f"   [WARNING] {corrupted_docs} documentos con encoding corrupto")
            else:
                print(f"   [OK] Todos los documentos OK")
        
        return issues
        
    except Exception as e:
        print(f"[ERROR] Error al acceder a ChromaDB: {str(e)}")
        return [{'issue': f'Error ChromaDB: {str(e)}'}]

def audit_search_functionality():
    """Prueba búsquedas con palabras en español."""
    print("\n[SEARCH] 4. TEST DE BUSQUEDAS CON CARACTERES ESPAÑOLES")
    print("-" * 70)
    ...
    try:
        ...
        print("[INFO] RAGService no disponible todavia")
        return []
    ...
    print(f"[INFO] Ejecutando {len(test_queries)} busquedas de prueba...")
    ...
    try:
        ...
        for query in test_queries:
            try:
                # Intentar búsqueda usando el método correcto (async)
                import asyncio
                results = asyncio.run(rag_service.retrieve_context(query, n_results=3))
                
                if results and len(results.strip()) > 0:
                    print(f"   [OK] '{query}': Contexto encontrado ({len(results)} chars)")
                else:
                    print(f"   [INFO] '{query}': Sin contexto")
                    
            except Exception as e:
                issues.append({
                    'query': query,
                    'issue': f'Error en busqueda: {str(e)}'
                })
                print(f"   [ERROR] '{query}': Error - {str(e)}")
        
        if not issues:
            print("\n[OK] Todas las busquedas funcionan correctamente")
            
    except Exception as e:
        print(f"[ERROR] Error inicializando RAG: {str(e)}")
...
def generate_report(all_issues):
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("=" * 70 + "\n")
        f.write("AUDITORIA DE ENCODING - AGENTE TURISMO IA\n")
        ...
        if total_issues == 0:
            ...
            print("\n[OK] SISTEMA OK: No se detectaron problemas de encoding")
            print(f"[INFO] Reporte guardado en: {report_file}")
            
        else:
            f.write(f"RESULTADO: {total_issues} PROBLEMAS DETECTADOS\n\n")
            ...
            print(f"\n[WARNING] SE DETECTARON {total_issues} PROBLEMAS")
            print(f"[INFO] Reporte detallado en: {report_file}")
            ...
            print(f"[INFO] Detalles JSON en: {json_file}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n[INFO] Auditoria completada. Exit code: {exit_code}")
        sys.exit(exit_code)
    except KeyboardInterrupt:
        print("\n\n[WARNING] Auditoria cancelada por el usuario")
        sys.exit(130)
    except Exception as e:
