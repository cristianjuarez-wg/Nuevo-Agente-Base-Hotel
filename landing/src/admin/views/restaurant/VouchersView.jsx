...
export default function VouchersView() {
  ...
  const columns = [
    { key: 'code', label: 'Voucher', render: (v) => <span className="font-semibold text-brand-700">{v.code}</span> },
  ...
  const renderCard = (v) => (
    <div>
      <div className="mb-2 flex items-center justify-between">
        <span className="font-semibold text-brand-700">{v.code}</span>
      ...
      <div className="mt-2 flex items-center justify-between">
        <span className="text-sm font-semibold tabular-nums text-brand-700">{formatUSD(v.total_usd)}</span>
        <RedeemBtn v={v} />
      </div>
    </div>
  )
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {FILTERS.map((f) => (
          <button key={f.id} onClick={() => setFilter(f.id)} className={`rounded-full px-3.5 py-2 text-xs font-medium transition ${filter === f.id ? 'bg-brand-600 text-white shadow-card' : 'bg-white text-slatey hover:bg-brand-50'}`}>{f.label}</button>
        ))}
      </div>
      {loading ? (
