/**
 * API del NUCLEO (F6) — genérico, reusable en cualquier rubro.
 *
 * Cliente axios + interceptores de auth (JWT / X-Admin-Key), y las funciones que NO dependen
 * del negocio: auth, chat, conversaciones, takeover, uso/costos, conocimiento, agentes,
 * temas, analíticas, contactos, demo.
 *
 * Lo hotelero vive en `api/hotel.js`. `services/api.js` re-exporta ambos para que los ~48
...
// Exportado para que el modulo de dominio (api/hotel.js) reuse el MISMO cliente
// con sus interceptores de auth, en vez de crear otro.
